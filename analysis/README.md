...
> **⚠️ 下表低估了推理型模型的成本。** 首次实跑（MXL，DeepSeek V4-Pro 全套）实测
> **19 次调用 / 输入 173,510 / 输出 58,570 token / $0.1264**——输出是估算值的近 5 倍，
> 因为 V4-Pro 是推理模型，思考 token 按输出计费。下表按「10.3 万输入 + 1.2 万输出」推算，
> 对 Haiku、Flash 这类非推理模型仍大致准确，**对推理模型（DeepSeek V4-Pro、Opus 5、
> GPT-5.5 等）请按约 2 倍看**。以每次跑完打印的实测值为准，别信估算。
